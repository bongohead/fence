...
def visualize_fence(
    ...
    fig = px.imshow(
        ...
        .update_layout(
            ...
            plot_bgcolor = 'white', paper_bgcolor = 'white', margin = dict(l = 100, r = 0, t = 50, b = 20),
    ...
    for fname, fdim in fence_dict.items():
        ...
        fig\
            ...
            .add_annotation(
                ...
                text =  fname + ' (<span style="text-decoration:overline">D<sub>f</sub></span> = ' + str(test_mat[:, start_ix:end_ix].mean().round(2)) + ')',
...
def visualize_fence_by_layer(
    ...
    """
    Call generate_multiple_outputs and graph each hidden state layer seperately

    Params:
        @decoded_text: A list of output strings
        @hks: The list of transformer layers outputs.
        @layers: A list of transformer layer indices to graph. Indices start from 1, not 0.
        @fence_dict: The FENCE feature dictionary.
        @start_dim: The starting dimension (1-indexed) to display the graph on. If None, sets to the minimum dimension in fence_dict minus 20.
        @end_dim: The ending dimension (1-indexed) to display the graph on. If None, sets to the maximum dimension in fence_dict plus 20.
        ...
        @max_range: The maximum (green) FENCE value, or a list of values equal to the length of layers
    """
    final_fig = make_subplots(len(layers) // 2 + 1, cols = 2) 
    ...
    for i, l in enumerate(layers):
        ...
        subfig = px.imshow(draw_mat, zmin = min_range - (max_range - min_range)/2, zmax = max_range + (max_range - min_range)/2)
        final_fig\
            .add_trace(subfig.data[0], row = row, col = col)\
            .update_yaxes(title_text = 'H<sub>' + str(l) + '</sub>', autorange = 'reversed', tickvals = list(range(len(custom_y))), ticktext = custom_y, tickfont = {'size': 10.5}, row = row, col = col)\
            .update_xaxes(showticklabels = False, row = row, col = col)
        
        start_ix = test_mat.shape[1] - np.sum([v for _, v in feature_dict.items()]) - start_dim
        for fname, fdim in feature_dict.items():
            final_fig\
                .add_shape(
                    row = row, col = col, type = 'rect', x0 = start_ix - .5, y0 = -.5, x1 = start_ix + fdim - .5, y1 = test_mat.shape[0] - .5, 
                    xref = 'x', yref = 'y', line = {'color': 'black', 'width': 2}
                    )\
                .add_annotation(
                    row = row, col = col,
                    textangle = 0, x = (start_ix * 2 + fdim - 1)/2, y = -2, xref = 'x', yref = 'y',
                    text = fname +  ' (<span style="text-decoration:overline">D<sub>f</sub></span> = ' + str(test_mat[:, start_ix:(start_ix + fdim)].mean().round(2)) + ')', 
                    showarrow = False, font = {'color': 'black', 'size': 9}, align = 'center'
                )
            start_ix = start_ix + fdim
    ...
    final_fig\
        .update_layout(
            height = 800,
            plot_bgcolor = 'white', paper_bgcolor = 'white', margin = dict(l = 100, r = 0, t = 50, b = 20),
            coloraxis = dict(colorscale = custom_colorscale, cmin = min_range - (max_range - min_range)/2, cmax = max_range + (max_range - min_range)/2),
            coloraxis_colorbar = {'orientation': 'h', 'yanchor': 'top', 'xanchor': 'right', 'y': 1.1, 'x': 1.0, 'title': None, 'len': 0.2, 'thickness': 10, 'tickfont': {'size': 9}}
        )
    
    return final_fig

@torch.no_grad()
def visualize_fence_differentials(
    decoded_text: list[str], 
    hks: list[np.ndarray], 
    layers: list[int], 
    fence_dict: dict,
    ...
    """
    Visualize the differentials between each layer

    Params:
        @decoded_text: A list of output strings
        @hks: The list of transformer layers outputs.
        @layers: A list of transformer layer indices to graph. Indices start from 1, not 0.
        @feature_dict: A FENCE feature dictionary.
        @start_dim: The starting dimension to display the graph on.
        @min_range: The minimum (red) FENCE value, or a list of values equal to the length of layers
        @max_range: The maximum (green) FENCE value, or a list of values equal to the length of layers
    """
    final_fig = make_subplots(len(layers) // 2 + 1, cols = 2)    
    ...
    for i, l in enumerate(layers):
        ...
        test_mat = hks[l - 1] - hks[l - 2]
        draw_mat = test_mat[:, start_dim:]
        custom_y = decoded_text[:-1]
        ...
        final_fig\
            .add_trace(subfig.data[0], row = row, col = col)\
            .update_yaxes(title_text = 'H<sub>' + str(l) + '</sub>', autorange = 'reversed', tickvals = list(range(len(custom_y))), ticktext = custom_y, tickfont = {'size': 10.5}, row = row, col = col)\
            .update_xaxes(showticklabels = True, row = row, col = col)
        ...
        for fname, fdim in feature_dict.items():
            final_fig\
                ...
                .add_annotation(
                    row = row, col = col,
                    textangle = 0, x = (start_ix * 2 + fdim - 1)/2, y = -2, xref = 'x', yref = 'y',
                    text = fname + ' (<span style="text-decoration:overline">ΔD<sub>f</sub></span> = ' + str(test_mat[:, start_ix:(start_ix + fdim)].mean().round(2)) + ')', 
                    showarrow = False, font = {'color': 'black', 'size': 9}, align = 'center'
                )
